...
// Ported from vite-plugins/rayontaraRecoveryApiPlugin.ts's '/api/rayontara/recovery' handler.
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const records = await fetchRecoveryForCodesWithRetry(credsFromEnv(), RAYONTARA_EMP_CODES, selectedMonth);
