...
// CONFIRMED LIVE on 2026-09-17 — the earlier "always empty" result wasn't a data-availability gap,
// it was this request sending the wrong flags: `excludeZero` and `IsAR` must be `"1"` (blank
// silently returns zero rows, no error) and `Option` must be `"All"` (blank/"Pending" only surfaces
...
// Company-wide fetch — mirrors Recovery/TDS/Leave's date-scoping, but as a single bulk query (like
// Meal) rather than per-EmpCode, since the documented request shape takes a date range plus optional
// filters, not a codes array. RequestedDateFrom/To is sent as a server-side pre-filter, but every
// record is ALSO re-checked client-side against its own Request Date (requirement: "no
// reimbursement from another month is displayed") — this doesn't assume the server's date-range
// filter is honored/inclusive/exclusive the way we expect, it just narrows the payload, and the
// authoritative per-record field decides month membership. `excludeZero: "1"` and `IsAR: "1"` are
// required — blank returns zero rows with no error. `Option: "All"` is required too — blank/
// "Pending" only returns requests still awaiting approval; "All" returns every status, which is why
// approval is then checked client-side (IsApproved/IsDisapproved) instead of trusting the query to
// have already filtered to payable claims. Amounts are summed per employee code for every APPROVED
// request whose Request Date falls in the selected month, in case someone has more than one approved
// item in the month (chair + desk + internet, say), same reasoning as Loan Advance's
...
async function fetchWfhReimbursements(
  ...
  for (const raw of raws) {
    ...
    if (parsedDate === null) {
      // eslint-disable-next-line no-console
      console.error('[rayontara-wfh-api] Request Date could not be parsed — update parseApiDate in rayontaraWfhApiPlugin.ts:', raw);
      continue;
    }
