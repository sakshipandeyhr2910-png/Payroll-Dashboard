// Duplicated from api/_lib/payrollCompute.ts (identical logic, ported from
// src/utils/{professionalTax,loanDeduction,arrearCalculation,attendance,month}.ts) so
// employeePayrollPlugin.ts doesn't cross the vite-plugins/api tsconfig-project boundary — same
// reasoning as vite-plugins/overseasEntityMapping.ts. Keep all three copies in sync if the
// underlying business rules ever change.
import type { LoanAdvanceRecord } from './rayontaraLoanApiPlugin';
import type { ArrearRecord } from './rayontaraArrearApiPlugin';
