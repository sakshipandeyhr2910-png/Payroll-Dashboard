// Duplicated from src/utils/overseasEntityMapping.ts (identical content) so employeeAuthPlugin.ts
// doesn't cross a tsconfig project boundary (vite-plugins/ builds under tsconfig.node.json, which
// doesn't include src/) — same reasoning api/_lib/ already follows for its own copies of shared
// logic. Keep both copies in sync if the routing rules ever change.
//
